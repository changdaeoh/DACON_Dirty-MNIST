...
성능이 우수하였던 `InceptionResnet v2, DenseNets, EfficientNetsB4,B5` 위주로 튜닝 진행. <br/>
...
초반에는 adaptive learning rate 기반 optimizer 사용 시 learning rate scheduling의 영향력이 크지 않을 것이라 생각했으나<br/>
대회 후반부로 갈 수록 LR scheduling이 생각보다 model performance에 critical함을 깨달았고,<br/>
코사인감쇄+재시작, 구간별 고정 감쇄, 성능기반 감쇄 등의 방법으로 epoch별 학습률을 조절해보았다.<br/>
...
아쉽게도 colab의 runtime limitation 때문에 이는 이뤄지지 못했다.<br/>
